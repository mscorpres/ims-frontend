...
export const materialInWithoutPo = async (values, fileName, vendorType) => {
  ...
  const payload = {
    ...
    misAmount: values.components.map((r) => r.misAmount),
    insuranceAmount: values.components.map((r) => r.insuranceAmount),
    taxableValue: values.components.map((r) => r.taxableValue),
    customDuty: values.components.map((r) => r.customDuty),
    totalAmount: values.components.map((r) => r.totalAmount),
    freightAmount: values.components.map((r) => r.freightAmount),
    finalRate: values.components.map((r) => r.finalRate),
